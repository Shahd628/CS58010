...
def load_client(args, client_id, data, data_dir, message_pool, device):
    ...
    elif args.fl_algorithm == "fedalaghn":
        from openfgl.flcore.fedalaghn.client2 import FedALAGHNClient
        return FedALAGHNClient(args, client_id, data, data_dir, message_pool, device)
    elif args.fl_algorithm == "fedalare":
        from openfgl.flcore.fedalare.client import FedALAreClient
        return FedALAreClient(args, client_id, data, data_dir, message_pool, device)
...
def load_server(args, global_data, data_dir, message_pool, device):
    ...
    if args.fl_algorithm == "isolate":
        ...
        from openfgl.flcore.fedalaghn.server2 import FedALAGHNServer
        return FedALAGHNServer(args, global_data, data_dir, message_pool, device)
    elif args.fl_algorithm == "fedalare":
        from openfgl.flcore.fedalare.server import FedALAreServer
        return FedALAreServer(args, global_data, data_dir, message_pool, device)
    elif args.fl_algorithm == "fedavg":
